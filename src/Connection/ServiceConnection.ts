    ...
    OnMessage = (_message: MessageEvent): void => {
        ...
        switch (looseData.action) {
            ...
            case ActionCode.CLOSE_TO_SWIPE: {
                const cursor = TouchFree.GetCurrentCursor();
                cursor?.ShowCloseToSwipe();
                cursor?.SetSwipeDirection(looseData.content as SwipeDirection);
                InputActionManager.HandleCloseToSwipe(looseData.content as SwipeDirection);
                break;
            }
        }
    };
