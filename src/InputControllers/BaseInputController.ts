...
export abstract class BaseInputController {
    ...
    constructor() {
        if (!BaseInputController.Instantiated) {
            ...
            InputActionManager.instance.addEventListener('TransmitCloseToSwipe', ((e: CustomEvent<SwipeDirection>) => {
    ...
    protected HandleCloseToSwipe(_direction?: SwipeDirection): void {}

    disconnect() {
        this.HandleInputActionCallback?.UnregisterEventCallback();
        InputActionManager.instance.removeEventListener('TransmitCloseToSwipe', ((e: CustomEvent<SwipeDirection>) => {
            this.HandleCloseToSwipe(e.detail);
        }) as EventListener);
